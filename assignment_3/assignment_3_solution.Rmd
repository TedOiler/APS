...
1.   $X$ and $Y$ are independent -which is true since they are different patients.
...
According to the two-sample randomization test based on the sample means, the *p-value* is $0.009$, and it was calculated for $1000$ iterations. This is significant on the $\alpha = 5\%$ significance level, which means that we **reject** the null hypothesis. Therefore the two random variables **do not have** the same mean.
